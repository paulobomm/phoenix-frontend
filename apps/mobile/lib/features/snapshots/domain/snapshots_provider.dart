...
import '../data/models/compare_result_model.dart';
...
final snapshotsProvider = FutureProvider.autoDispose<List<SnapshotModel>>((ref) async {
  final selectedGame = ref.watch(selectedGameProvider);
  final repo = ref.read(snapshotsRepositoryProvider);
  return repo.getSnapshots(selectedGame?.id ?? '');
});

final snapshotDetailProvider = FutureProvider.autoDispose.family<SnapshotModel?, String>((ref, id) async {
  final repo = ref.read(snapshotsRepositoryProvider);
  return repo.getSnapshot(id);
});

final compareResultProvider = StateProvider<CompareResultModel?>((ref) => null);
