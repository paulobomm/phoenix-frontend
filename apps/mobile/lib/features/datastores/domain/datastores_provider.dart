...
final datastoresProvider = FutureProvider.autoDispose<List<DataStoreModel>>((ref) async {
  final selectedGame = ref.watch(selectedGameProvider);
  final repo = ref.read(datastoresRepositoryProvider);
  return repo.getDataStores(selectedGame?.id ?? '');
});

final entriesProvider = FutureProvider.autoDispose.family<List<EntryModel>, String>((ref, datastoreId) async {
  final repo = ref.read(datastoresRepositoryProvider);
  return repo.getEntries(datastoreId);
});
